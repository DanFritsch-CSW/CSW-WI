...
// WR Pick Location Lot Check digest — MANUAL TEST ONLY. Added 2026-07-31
// as the sibling to wr-pickcheck-digest-run.cjs (see that file's header,
// and lib/wr-pickcheck-digest-shared.cjs's header, for why this split
// exists).
//
...
async function runTest() {
  ...
  const settingsRows = await sbFetch(
    `prepick_notify_settings?facility=eq.wr&dashboard_type=eq.pick_check&select=front_conversation_id`
  )
