...
// Direct SharePoint share URLs — no search, no site/drive resolution needed.
// Each URL encodes a specific file in the Employee site.
...
// Converts a SharePoint sharing URL to a Graph API drive item reference.
// Cache per facility so warm invocations skip the round-trip.
const _driveCache = {}
async function getDriveRef (facility, token) {
  ...
  // Encode the share URL to a Graph shares ID: base64url with u! prefix
  ...
  const item = await graph(`/shares/${shareId}/driveItem`, token)
  _driveCache[facility] = {
...
function parseRows (headerRow, dataRows, prefix) {
  ...
  const TYPE_NORM = {
    ...
    'warehouse ops damage': 'Warehouse Ops Damage',
    'warehouse ops damage ': 'Warehouse Ops Damage',
    'disposal': 'Disposal', 'transfer': 'Transfer',
    'dsd': 'DSD', 'cycle count': 'Cycle Count',
  }
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const rowIndex = i + 2 // Excel row number (1-indexed, row 1 = headers)
    ...
    incidents.push({
      ...
      incidentType,
      reason: reasonRaw.charAt(0).toUpperCase() + reasonRaw.slice(1),
      damageType: '',
...
exports.handler = async function (event) {
  ...
  // Determine facility from query string (defaults to 'mad' for backward compat)
  const params   = event.queryStringParameters || {}
  const facility = (params.facility || 'mad').toLowerCase()
  const config   = FACILITY_CONFIG[facility]
  if (!config) {
    return { statusCode: 400, headers: cors, body: JSON.stringify({ error: `Unknown facility: ${facility}. Use cal, ken, or mad.` }) }
  }
  ...
  try {
    ...
    if (!range.values || range.values.length < 2) {
      return { statusCode: 200, headers: cors, body: JSON.stringify({ incidents: [], count: 0, facility }) }
    }
