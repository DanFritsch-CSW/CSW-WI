...
// WR "Cases To Pick" digest — MANUAL TEST ONLY. Added 2026-07-31 as the
// sibling to wr-cases-digest-run.cjs (see that file's header, and
// lib/wr-cases-digest-shared.cjs's header, for why this split exists).
//
...
async function runTest() {
  ...
  const settingsRows = await sbFetch(
    `prepick_notify_settings?facility=eq.wr&dashboard_type=eq.cases_to_pick&select=front_conversation_id`
  )
