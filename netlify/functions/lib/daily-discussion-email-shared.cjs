...
// Shared core for Daily Discussion EMAIL draft creation — added 2026-09-01
// per Dan's ask. Deliberately NOT connected to the internal Front
// discussion feature (front-daily-discussion-run.cjs) — that stays
// untouched, this is a completely separate, fresh email draft. Dan's own
// words: "not sure where the 'replace' internal front discussion aspect
// comes from... This would be a fresh email."
//
// Directly mirrors lib/cmm-outbound-draft-shared.cjs (same structure,
// same runDigest({isManualTest, facility}) shape, same Front draft-
// creation calls) with two differences:
//
//   1. FACILITY IS SELECTABLE, not hardcoded. cmm-outbound-draft-shared.cjs
//      already accepted `facility` as a runtime parameter and its
//      WAREHOUSE_ID map already covered all 5 facilities — CMM Outbound
//      just never exposed a facility picker in the UI (CMM_FACILITY='cal'
//      constant in Settings.jsx). This feature's Settings UI DOES expose
//      a facility dropdown (reusing the one already in the Daily
//      Discussions editor), per Dan's explicit ask that facility choice
//      be "editable based on the UI."
//
//   2. THE APPOINTMENT QUERY IS GENERAL, not CMM/Palermo's-specific. CMM
//      Outbound filters to one carrier + one customer + outbound-only.
//      This is meant to be a full daily appointment list for the
//      facility (both directions, any customer) — the natural content
//      for a general daily-discussion-style email, not a single-customer
//      digest. See queryAppointments() below.
//
// Reuses the SAME prepick_notify_settings table (dashboard_type=
// 'daily_discussion_email' instead of 'cmm_outbound_appts' — that table
// was already facility+dashboard_type keyed, no schema change needed) and
// the SAME notification_recipients table for internal-discussion
// followers (list_name=`daily_discussion_email_<facility>`). Only new
// table: daily_discussion_email_recipients (TO/CC), kept separate from
// cmm_outbound_email_recipients so the two features' recipient lists
// never collide for a shared facility like 'cal'.
...
function buildDraftHtml(rows, dateObj, facilityDisplay) {
  ...
  const rowsHtml = rows.map(r => `
    <tr>
      ...
      <td style="padding:4px 10px;border-bottom:1px solid #ddd;">${escapeHtml(r.carrier_name || '—')}</td>
    </tr>`).join('')

  return `
    <div style="font-family:Arial,sans-serif;font-size:13px;color:#222;">
      <h3 style="margin:0 0 4px;">Daily Discussion — ${escapeHtml(facilityDisplay)}</h3>
      <p style="margin:0 0 12px;color:#555;">${escapeHtml(formatHeaderDate(dateObj))}</p>
      <p style="margin:0 0 12px;"><strong>Total Appointments: ${count}</strong></p>
      <table style="border-collapse:collapse;width:100%;max-width:640px;">
        <thead>
          <tr>
            ...
            <th style="text-align:left;padding:4px 10px;border-bottom:2px solid #333;">Carrier</th>
          </tr>
        </thead>
        <tbody>${rowsHtml || '<tr><td colspan="5" style="padding:8px 10px;">No appointments scheduled.</td></tr>'}</tbody>
      </table>
    </div>`
}

// General appointment list for the facility/date — ANY customer, BOTH
// directions, unlike CMM Outbound's single-carrier/single-customer/
// outbound-only filter. Still excludes Cancelled, same as CMM Outbound.
...
async function runDigest({ isManualTest, facility }) {
  ...
  const settingsRows = await sbFetch(
    `prepick_notify_settings?facility=eq.${facility}&dashboard_type=eq.${DASHBOARD_TYPE}&select=notify_hour,notify_minute,notify_days,active,last_sent_date,discussion_comment,author_teammate_id,from_channel_id`
  )
  ...
  if (!settings.author_teammate_id) {
    return { ok: false, reason: 'No author_teammate_id configured — set a Draft Author in Settings > Daily Discussions' }
  }

  const [emailRows, discussionRows] = await Promise.all([
    sbFetch(`daily_discussion_email_recipients?facility=eq.${facility}&active=eq.true&select=email,role`),
    sbFetch(`notification_recipients?list_name=eq.daily_discussion_email_${facility}&active=eq.true&select=front_teammate_id`),
  ])
  ...
  if (to.length === 0) {
    return { ok: false, reason: 'No active TO recipients configured in Settings > Daily Discussions' }
  }

  const appts = await queryAppointments(warehouseId, date)
  const facilityDisplay = FACILITY_DISPLAY[facility] || facility.toUpperCase()
  const subject = `Daily Discussion Draft — ${facilityDisplay} — ${date}`
