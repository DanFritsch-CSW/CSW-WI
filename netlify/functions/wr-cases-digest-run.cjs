...
// Nightly WR "Cases To Pick" digest — SCHEDULED TICK ONLY.
//
...
// wr-cases-digest-test.cjs instead. Both require
// lib/wr-cases-digest-shared.cjs so the actual digest logic (message
// building, Front posting) lives in exactly one place.
//
// See lib/wr-cases-digest-shared.cjs for the fuller original feature
...
const {
  ...
  postDigest,
} = require('./lib/wr-cases-digest-shared.cjs')
...
exports.handler = async function (event) {
  ...
  if (!isScheduled) {
    return { statusCode: 405, headers: NO_CACHE_HEADERS, body: JSON.stringify({ error: 'Scheduled invocation only — use wr-cases-digest-test for manual sends' }) }
  }
  try {
