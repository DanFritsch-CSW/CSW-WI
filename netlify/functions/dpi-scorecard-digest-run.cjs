...
// DPI Putaway Scorecard daily digest — SCHEDULED TICK ONLY. Duplicated
// from jdf-scorecard-digest-run.cjs 2026-08-25 — same split-function
// pattern as every other digest in this app (Netlify blocks direct HTTP
// invocation of any function carrying a `schedule` in netlify.toml).
...
exports.handler = async function (event) {
  ...
  if (!isScheduled) {
    return { statusCode: 405, headers: NO_CACHE_HEADERS, body: JSON.stringify({ error: 'Scheduled invocation only — use dpi-scorecard-digest-test for manual sends' }) }
  }
  try {
