...
// F8 Open Positions morning digest — SCHEDULED TICK ONLY. Added
// 2026-09-04, same split-function pattern as every other digest in this
...
// for the full story/repro). This file keeps the `schedule` and only
// handles the cron tick; "Send test digest now" calls the sibling
// function f8-open-positions-digest-test.cjs instead, which has no
...
async function runScheduledDigest() {
  ...
  const notifyHour = row.notify_hour ?? 7
...
exports.handler = async function (event) {
  ...
  if (!isScheduled) {
    return { statusCode: 405, headers: NO_CACHE_HEADERS, body: JSON.stringify({ error: 'Scheduled invocation only — use f8-open-positions-digest-test for manual sends' }) }
  }
  try {
