...
// F8 Open Positions morning digest — MANUAL TEST ONLY. Added 2026-09-04,
// sibling to f8-open-positions-digest-run.cjs (see that file's header,
// and lib/f8-open-positions-digest-shared.cjs's header, for why this
// split exists). This file deliberately has NO `schedule` entry in
// netlify.toml, so Netlify allows the browser to POST to it directly.
// Always sends immediately for today's content date regardless of
// time/day/active, and does not touch last_sent_date — same "test
// doesn't interfere with the real scheduled send" behavior as every
// other *-digest-test.cjs.

const {
  FACILITY, DASHBOARD_TYPE,
  SUPABASE_URL, SUPABASE_KEY, FRONT_TOKEN, MOTHERDUCK_TOKEN,
  sbFetch, runDigest,
} = require('./lib/f8-open-positions-digest-shared.cjs')
