...
// WR Pick Location Lot Check digest — SCHEDULED TICK ONLY.
//
// Split 2026-07-31 (later same day as the message-format simplification),
// same fix pattern as fefo-digest-run.cjs / prepick-digest-run.cjs: Netlify
// blocks direct HTTP invocation of a function that carries a `schedule`,
// which is why "Send test digest now" 403ed here. This file keeps the
// `schedule` and ONLY handles the cron tick; "Send test digest now" now
// calls the sibling wr-pickcheck-digest-test.cjs instead. Both require
// lib/wr-pickcheck-digest-shared.cjs so the actual digest logic (message
// building, Front posting) lives in exactly one place.
//
// See lib/wr-pickcheck-digest-shared.cjs for the fuller original feature
// history — none of that changed, only where the manual-test entry point
// lives.
...
const {
  ...
  postDigest,
} = require('./lib/wr-pickcheck-digest-shared.cjs')
...
exports.handler = async function (event) {
  ...
  if (!isScheduled) {
    return { statusCode: 405, headers: NO_CACHE_HEADERS, body: JSON.stringify({ error: 'Scheduled invocation only — use wr-pickcheck-digest-test for manual sends' }) }
  }
  try {
