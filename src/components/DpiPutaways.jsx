...
import { RACK_TYPE, RACK_GROUP, CUSTOMER_NAMES } from '../lib/dpiPutawaysLocations.js'
import { fetchDpiPutaways } from '../lib/dpiPutaways.js'
import { pct, classifyLocation, getWindowStart, windowLabel } from '../lib/dpiPutawaysLogic.js'
import NotifySettingsPanel from './NotifySettingsPanel.jsx'

// ─── DPI Putaways ───────────────────────────────────────────────────────
// Visibility tool for DPI slotting at Madison (F5 + F8 zones): how well
// putaways are following single-SKU discipline within DPI's own product,
// who's been moving pallets into currently-mixed locations, and recent raw
// volume by employee/zone-aisle. Duplicated from JdfPutaways.jsx 2026-08-25
// per Dan's request ("duplicate the JDF Putaway tab, make it identical in
// nature"), with three scope decisions Dan confirmed before this was built
// -- see motherduck-dpi-putaways.cjs's header for the full detail:
//   1. DPI hard-coded to F5 + F8 only (not building-wide, not F2 -- DPI has
//      zero active F2 inventory today).
//   2. The second scorecard layer is "also same RECEIPT date", not "same
//      MAN date" -- DPI's lot codes carry no encoded manufacture date the
//      way JDF's do, but lot.receive_date is populated and usable.
//   3. Zone-qualified aisle keys ("F5-A", "F8-A") everywhere, since F5 and
//      F8 are physically different racks that happen to share aisle
//      letters -- a bare-letter key would silently merge them.
//
// LIVE DATA: comes from netlify/functions/motherduck-dpi-putaways.cjs on
// every load -- see that file's header for the query.
//
// SCOPE NOTE (2026-08-25 build): this first pass intentionally does NOT
// include DPI equivalents of JdfLpLocator.jsx or JdfSameItemReference.jsx
// (the standalone LP-lookup and reference-table tools bolted onto the
// bottom of the JDF tab), or the per-employee move-report PDF in the
// digest. Left out to keep this change reviewable -- straightforward to
// port over later the same way if Dan wants full parity.
//
// Comingling in the 7-deep drive-in racks (F8 A/G/H) is NOT scored here --
// same reasoning as the JDF tab: using full lane depth for other customers
// behind faster DPI movers is deliberate. F5 is uniformly 4-deep (per Dan,
// confirmed live 2026-08-25) so there's no comparable "shared depth" case
// there, but the "About shared lanes" note below still applies to F8.

const STATUS_ORDER = { clean: 0, mixed_date: 1, mixed_item: 2 }

export default function DpiPutaways() {
  ...
  const enriched = useMemo(
    () =>
      rawLocations.map(loc => {
        const [location, zoneAisle, dpiLp, distinctMaterials, distinctRecvDates, earliest, latest, otherLp, otherCustomers] = loc
        return {
          location, aisle: zoneAisle, dpiLp, distinctMaterials, distinctRecvDates, earliest, latest, otherLp,
          otherCustomers: otherCustomers ? otherCustomers.split(',') : [],
          isShared: otherLp > 0,
          status: classifyLocation(loc),
          rackGroup: RACK_GROUP[zoneAisle] || 'unknown',
        }
      }),
    [rawLocations]
  )
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(map.values())
      .map(g => ({
        ...g,
        rackType: groupBy === 'aisle' ? RACK_TYPE[g.key] : (g.key === '7-deep' ? '4-high lanes' : g.key === '4-deep' ? '4-deep lanes' : '7-high lanes'),
  ...
  const totals = useMemo(() => {
    ...
    const avgLanePallets = deep.length
      ? Math.round((deep.reduce((s, r) => s + r.dpiLp + r.otherLp, 0) / deep.length) * 10) / 10
      : 0
  ...
  const drillLocations = useMemo(() => {
    ...
    rows = [...rows].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'dpiLp': av = a.dpiLp; bv = b.dpiLp; break
  ...
  // ── Recent-activity window (employee + zone-aisle raw volume) ─────────
  ...
  const recentByAisle = useMemo(() => {
    ...
    for (const [, location, status, ts] of employeeEvents) {
      const t = new Date(ts)
      if (t < windowStart || t > now) continue
      const aisle = location.slice(0, 2) + '-' + location.charAt(2)
    ...
    for (const [aisle, ts] of allMoves) {
      const t = new Date(ts)
      if (t < windowStart || t > now) continue
    ...
    return Array.from(aisles)
      ...
      .sort((a, b) => b.total - a.total)
  }, [employeeEvents, allMoves, windowStart, now])
  ...
  if (loading && !data) {
    return <div style={{ padding: 20, fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>Loading DPI Putaways…</div>
  }
  ...
  const daily = data?.dailyScorecard ?? { date: null, totalReceived: 0, putAway: 0, stillStaged: 0, sameItemTier: 0, sameItemTierRecvDate: 0 }
  const building = data?.buildingWide ?? { totalActive: 0, sameItemTier: 0, sameItemTierRecvDate: 0 }
  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        <div className="section-label" style={{ marginBottom: 4 }}>DPI Putaways — Same Item, Same Tier</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', maxWidth: 640 }}>
          F5 + F8 only. Same item, same tier. Two numbers: how the team is executing today, and where the whole building stands right now.
        </div>
      ...
      {/* ── Row 2: layer — also same receipt date ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 12, marginBottom: 12 }}>
        <div style={{ ...scoreCardStyle, borderTop: '2px solid var(--blue)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={scoreLabelStyle}>Daily — Also Same Receipt Date</div>
              <div style={{ ...scoreValueStyle, color: 'var(--blue)' }}>{pct(daily.sameItemTierRecvDate, daily.putAway)}%</div>
            ...
            <div style={{ textAlign: 'right', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.8 }}>
              ...
              <div style={{ color: 'var(--blue)' }}>{daily.sameItemTierRecvDate} also same date</div>
            </div>
          </div>
          <div style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 10 }}>
            {daily.sameItemTierRecvDate} of the {daily.sameItemTier} same item/tier pallets
            ({pct(daily.sameItemTierRecvDate, daily.sameItemTier)}%) also share a receipt date — this is "same intake cohort,"
            not a shelf-life/FEFO signal (DPI's lot codes don't encode a manufacture date the way JDF's do).
          </div>
        </div>

        <div style={{ ...scoreCardStyle, borderTop: '2px solid var(--blue)' }}>
          <div style={scoreLabelStyle}>Building-Wide — Also Same Receipt Date</div>
          <div style={{ ...scoreValueStyle, color: 'var(--blue)' }}>{pct(building.sameItemTierRecvDate, building.totalActive)}%</div>
          ...
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.9 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Also same receipt date</span><strong style={{ color: 'var(--blue)' }}>{building.sameItemTierRecvDate}</strong>
            </div>
          </div>
        </div>
      </div>
      ...
      <NotifySettingsPanel
        facility="mad"
        dashboardType="dpi_putaway_scorecard"
        functionName="dpi-scorecard-digest-test"
        contentDateLabel="today"
        showSkipToNextValidDay={false}
        digestDescription="Posts the Daily Putaway Scorecard (same item/tier + also same receipt date, plus a still-staged count) and the Building-Wide baseline as a Front comment."
      />

      <div style={{ ...cardStyle, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 8 }}>
        <strong style={{ color: 'var(--text-primary)' }}>About shared lanes:</strong> in F8's 7-deep drive-in racks (A, G, H),
        placing a slower-moving customer's pallets behind faster DPI movers is a deliberate way to use full lane depth —
        it isn't counted against DPI here. F5 is uniformly 4-deep and doesn't have that same depth-sharing pattern.
      </div>

      <div className="cal2-tab-row">
        <button className={`cal2-tab${groupBy === 'aisle' ? ' active' : ''}`} onClick={() => { setGroupBy('aisle'); setSelectedGroup(null) }}>By zone/aisle</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ ...cardStyle, borderTop: '2px solid var(--yellow)' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>same SKU, diff receipt date</div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-dim)' }}>F8 7-deep aisles — utilization</div>
        </div>
      </div>

      {worst && (
        <div style={{ ...cardStyle, marginBottom: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)' }}>
          {groupBy === 'aisle' ? 'Zone/aisle' : 'Rack type'} <strong style={{ color: 'var(--text-primary)' }}>{worst.key}</strong> ({worst.rackType}) has
          the lowest clean rate at <strong style={{ color: 'var(--text-primary)' }}>{worst.cleanPct}%</strong> — that's DPI's own
        ...
        <div style={{ marginBottom: 16, border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, background: 'var(--bg2)', padding: '8px 12px' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, whiteSpace: 'nowrap' }}>
              {groupBy === 'aisle' ? 'Zone/aisle' : 'Rack type'} {selectedGroup} · {drillLocations.length} of{' '}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input
                ...
                placeholder="Filter location, e.g. F8E45"
              ...
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'left', cursor: 'pointer' }} onClick={() => toggleSort('earliest')}>
                    Receipt date range {sortKey === 'earliest' && (sortDir === 'asc' ? '▲' : '▼')}
                  </th>
                  <th style={{ cursor: 'pointer' }} onClick={() => toggleSort('dpiLp')}>
                    DPI LPs {sortKey === 'dpiLp' && (sortDir === 'asc' ? '▲' : '▼')}
                  </th>
                  ...
                  <tr key={r.location}>
                    ...
                    <td style={{ textAlign: 'left' }}>
                      {r.isShared
                        ...
                        : <span style={{ color: 'var(--text-dim)' }}>DPI only</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 16, marginBottom: 16 }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', marginBottom: 10 }}>
          Mixed-slot pallets moved into their current F5/F8 location, traced to whoever performed the last move. Raw count,
                                  ...
                                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    <thead>
                                      <tr style={{ color: 'var(--text-dim)' }}>
                                        ...
                                        <th style={{ textAlign: 'left', padding: '2px 6px' }}>Receipt date</th>
        ...
        <div style={{ display: 'flex', gap: 14, fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--text-dim)', marginTop: 8 }}>
          ...
          <span><span style={{ display: 'inline-block', width: 8, height: 8, background: '#f0c14b', marginRight: 4, borderRadius: 2 }} />Mixed receipt date</span>
        </div>
      </div>

      {/* ── Raw pallets per zone/aisle ───────────────────────────────────── */}
      <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 16, marginBottom: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div className="section-label" style={{ marginBottom: 4 }}>Raw pallets per zone/aisle</div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-dim)' }}>{windowLabel(now)}</div>
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', marginBottom: 10 }}>
          All F5/F8 DPI pallet moves in this window, grouped by zone/aisle — clean landings alongside the mixed-slot moves from
            ...
            {recentByAisle.map(a => {
              ...
              return (
                <div key={a.aisle} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 100, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-secondary)', flexShrink: 0 }}>{a.aisle}</div>
                  <div style={{ flex: 1, display: 'flex', height: 18, borderRadius: 4, overflow: 'hidden', background: 'var(--bg3)' }}>
