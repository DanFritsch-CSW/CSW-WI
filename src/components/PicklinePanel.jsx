...
import { fetchPickSchedule, upsertPickScheduleRow, insertPickScheduleRow, deletePickScheduleRow, fetchPickerAssignments, upsertPickerAssignment, deletePickerAssignment } from '../lib/supabase.js'
...
function minsToDisplay(mins) {
  if (mins == null) return ''
  const h = Math.floor(mins / 60), m = mins % 60, ap = h < 12 ? 'am' : 'pm', hd = h % 12 || 12
...
function parseDisplayTime(str) {
  ...
  if (!m) return null
  let h = parseInt(m[1], 10); const min = m[2] ? parseInt(m[2], 10) : 0, ap = m[3]
  if (ap === 'pm' && h !== 12) h += 12
  if (ap === 'am' && h === 12) h = 0
...
function sheetToRows(wb, nameHint) {
  const key = Object.keys(wb.Sheets).find(k => k.toLowerCase().includes(nameHint.toLowerCase()))
  if (!key) return []
  return wb._XLSX.utils.sheet_to_json(wb.Sheets[key], { defval: '' })
}
function normalizeRouteNum(raw) { return String(raw ?? '').trim().replace(/^0+(\d)/, '$1') }
...
function buildScheduleMap(scheduleRows) {
  ...
  for (const row of scheduleRows) {
    const day = row.day_of_week, rt = String(row.route_number)
    if (!map[day]) map[day] = {}
    map[day][rt] = { seq: row.pick_seq, name: row.route_name || null, apptStart: row.appt_start_mins ?? null, apptEnd: row.appt_end_mins ?? null, live: (row.drop_or_live || '').toLowerCase() === 'live' }
  }
  return map
}
...
export function parsePicklineXlsx(wb, scheduleRows = []) {
  ...
  for (const row of sheetToRows(wb, 'Shortage')) {
    ...
    const date = normalizeDate(String(dateVal)), itemNum = String(row['Item Number'] ?? row['Material Lookup Code'] ?? '').trim()
    if (!itemNum) continue
  ...
  const dateRtSkuOrig = {}, dateRtSkuLines = {}
  ...
  if (hasShortages) {
    for (const date of allDates) {
      rtSkuShorted[date] = {}
      const rtSkuOrig = dateRtSkuOrig[date], dow = dateToDow(date)
      const seqForDay = (dow && schedMap[dow]) ? schedMap[dow] : {}
      const sortedRts = Object.keys(rtSkuOrig).sort((a, b) => { const sa = seqForDay[a]?.seq ?? null, sb = seqForDay[b]?.seq ?? null; if (sa !== null && sb !== null) return sa - sb; if (sa !== null) return -1; if (sb !== null) return 1; return parseInt(a,10) - parseInt(b,10) })
      ...
      for (const [itemNum, available] of Object.entries(dateShortages)) {
        ...
        for (const rt of sortedRts) { const ordered = rtSkuOrig[rt][itemNum] || 0; if (ordered === 0) continue; const allocated = Math.min(ordered, remaining); rtSkuShorted[date][rt][itemNum] = ordered - allocated; remaining = Math.max(0, remaining - allocated) }
      }
    }
  }
  const dateRouteMaps = {}
  for (const date of allDates) {
    dateRouteMaps[date] = {}
    const rtSkuLines = dateRtSkuLines[date], rtSkuOrig = dateRtSkuOrig[date], shortedForDate = rtSkuShorted[date] ?? {}
    for (const rt of Object.keys(rtSkuLines)) {
      const rtShorted = shortedForDate[rt] ?? {}, rm = { gross: 0, alloc: 0, net: 0, shorted: 0, z: {} }
      for (const [matCode, lines] of Object.entries(rtSkuLines[rt])) {
        const origTotal = rtSkuOrig[rt][matCode] || 0, shortedTotal = rtShorted[matCode] || 0
        rm.gross += origTotal; rm.shorted += shortedTotal
        if (shortedTotal >= origTotal) continue
        let remainingShortage = shortedTotal
        const adjLines = [...lines].sort((a, b) => b - a).map(c => { if (remainingShortage <= 0) return c; const cut = Math.min(c, remainingShortage); remainingShortage -= cut; return c - cut })
        if (hasTieHigh) { const sku = skuMap[matCode]; for (const adjCases of adjLines) { if (adjCases <= 0) continue; if (sku && adjCases >= sku.fullPallet / 2) { rm.alloc += adjCases } else { rm.net += adjCases; if (sku && sku.zone > 0) rm.z[sku.zone] = (rm.z[sku.zone] || 0) + adjCases } } } else { rm.net += origTotal - shortedTotal }
      }
      dateRouteMaps[date][rt] = rm
    }
  }
  function summarize(routeMap, date) {
    const dow = dateToDow(date), seqForDay = (dow && schedMap[dow]) ? schedMap[dow] : {}
    const routes = Object.keys(routeMap).sort((a, b) => { const sa = seqForDay[a]?.seq ?? null, sb = seqForDay[b]?.seq ?? null; if (sa !== null && sb !== null) return sa - sb; if (sa !== null) return -1; if (sb !== null) return 1; return parseInt(a,10) - parseInt(b,10) }).map(rt => { const sched = seqForDay[rt]; return { rt, nm: sched?.name ?? ROUTE_NAMES[rt] ?? `Rt ${rt}`, cs: routeMap[rt].net, gross: routeMap[rt].gross, alloc: routeMap[rt].alloc, shorted: routeMap[rt].shorted, ready: sched?.apptStart ?? null, apptEnd: sched?.apptEnd ?? null, live: sched?.live ?? ROUTE_LIVE.has(rt), z: routeMap[rt].z } })
    return { routes, net_cs: routes.reduce((s,r)=>s+r.cs,0), alloc_cs: routes.reduce((s,r)=>s+r.alloc,0), shorted_cs: routes.reduce((s,r)=>s+r.shorted,0), gross_cs: routes.reduce((s,r)=>s+r.gross,0) }
  }
  const primary = summarize(dateRouteMaps[snapshotDate] ?? {}, snapshotDate)
  const next_dates = allDates.slice(1).map(date => { const s = summarize(dateRouteMaps[date], date); return { date, gross_cs: s.gross_cs, alloc_cs: s.alloc_cs, shorted_cs: s.shorted_cs, net_cs: s.net_cs } })
...
export function buildSnapshotFromOmni(casesRows, tieHighRows, shortageRows, date, scheduleRows = []) {
  ...
  for (const row of tieHighRows) {
    const code = String(row['Material Lookup Code'] ?? '').trim()
    if (!code) continue
    const zone = Number(row['Pickline Zones']) || 0, fp = Number(row['Full Pallet (t x h)']) || 0
    skuMap[code] = { zone, fullPallet: fp }
  }
  const hasTieHigh = Object.keys(skuMap).length > 0
  const shortageMap = {}
  ...
  if (hasShortages) {
    for (const d of allDates) {
      rtSkuShorted[d] = {}
      const rtSkuOrig = dateRtSkuOrig[d], dow = dateToDow(d)
      const seqForDay = (dow && schedMap[dow]) ? schedMap[dow] : {}
      const sortedRts = Object.keys(rtSkuOrig).sort((a, b) => { const sa = seqForDay[a]?.seq ?? null, sb = seqForDay[b]?.seq ?? null; if (sa !== null && sb !== null) return sa - sb; if (sa !== null) return -1; if (sb !== null) return 1; return parseInt(a,10) - parseInt(b,10) })
      ...
      for (const [itemNum, available] of Object.entries(dateShortages)) {
        ...
        if (totalOrdered === 0 || available >= totalOrdered) continue
        let remaining = available
        for (const rt of sortedRts) { const ordered = rtSkuOrig[rt][itemNum] || 0; if (ordered === 0) continue; const allocated = Math.min(ordered, remaining); rtSkuShorted[d][rt][itemNum] = ordered - allocated; remaining = Math.max(0, remaining - allocated) }
      }
    }
  }
  const dateRouteMaps = {}
  for (const d of allDates) {
    dateRouteMaps[d] = {}
    const rtSkuLines = dateRtSkuLines[d], rtSkuOrig = dateRtSkuOrig[d], shortedForDate = rtSkuShorted[d] ?? {}
    for (const rt of Object.keys(rtSkuLines)) {
      const rtShorted = shortedForDate[rt] ?? {}, rm = { gross: 0, alloc: 0, net: 0, shorted: 0, z: {} }
      for (const [matCode, lines] of Object.entries(rtSkuLines[rt])) {
        const origTotal = rtSkuOrig[rt][matCode] || 0, shortedTotal = rtShorted[matCode] || 0
  ...
  function summarize(routeMap, d) {
    const dow = dateToDow(d), seqForDay = (dow && schedMap[dow]) ? schedMap[dow] : {}
    const routes = Object.keys(routeMap).sort((a, b) => { const sa = seqForDay[a]?.seq ?? null, sb = seqForDay[b]?.seq ?? null; if (sa !== null && sb !== null) return sa - sb; if (sa !== null) return -1; if (sb !== null) return 1; return parseInt(a,10) - parseInt(b,10) }).map(rt => { const sched = seqForDay[rt]; return { rt, nm: sched?.name ?? ROUTE_NAMES[rt] ?? `Rt ${rt}`, cs: routeMap[rt].net, gross: routeMap[rt].gross, alloc: routeMap[rt].alloc, shorted: routeMap[rt].shorted, ready: sched?.apptStart ?? null, apptEnd: sched?.apptEnd ?? null, live: sched?.live ?? ROUTE_LIVE.has(rt), z: routeMap[rt].z } })
    return { routes, net_cs: routes.reduce((s,r)=>s+r.cs,0), alloc_cs: routes.reduce((s,r)=>s+r.alloc,0), shorted_cs: routes.reduce((s,r)=>s+r.shorted,0), gross_cs: routes.reduce((s,r)=>s+r.gross,0) }
  }
  const primary = summarize(dateRouteMaps[snapshotDate] ?? {}, snapshotDate)
  const next_dates = allDates.slice(1).map(d => { const s = summarize(dateRouteMaps[d], d); return { date: d, gross_cs: s.gross_cs, alloc_cs: s.alloc_cs, shorted_cs: s.shorted_cs, net_cs: s.net_cs } })
...
function fmt(m) {
  ...
  if (mm >= 60) { hh++; mm = 0 }
  const ap = hh < 12 ? 'am' : 'pm', h = hh > 12 ? hh - 12 : (hh === 0 ? 12 : hh)
  return `${h}:${String(mm).padStart(2, '0')}${ap}`
}
function buildTimeline(pickers, cpmh, targetCases = 0, hourOverrides = {}) {
  const tl = []; let cum = 0
  for (let idx = 0; idx < PACE.length; idx++) {
    const b = PACE[idx], effPickers = hourOverrides[idx]?.pickers ?? pickers, effCpmh = hourOverrides[idx]?.cpmh ?? cpmh
    const rate = (effPickers * effCpmh) / 60, bucketEnd = b.clockStart + (b.clockStart === 13*60 ? 30 : 60)
    for (let t = b.clockStart; t < bucketEnd; t++) {
      const inBreak = t < SHIFT_START || BREAKS.some(([bs, be]) => t >= bs && t < be)
      tl.push({ t, cum, picking: !inBreak, rate }); if (!inBreak) cum += rate
    }
  }
  const globalRate = (pickers * cpmh) / 60; let t = 13*60 + 30
  while (cum < targetCases && t < 19*60) { tl.push({ t, cum, picking: true, rate: globalRate }); cum += globalRate; t++ }
  tl.push({ t, cum, picking: false, rate: globalRate }); return tl
}
function casesToClock(tl, target) {
  for (let i = 0; i < tl.length - 1; i++) {
    const rate = tl[i].rate
    if (tl[i].picking && target >= tl[i].cum && target < tl[i].cum + rate) return tl[i].t + (target - tl[i].cum) / rate
  }
...
function getZoneDemand(routes) {
  const d = {}; routes.forEach(r => Object.entries(r.z||{}).forEach(([z,v]) => { d[+z]=(d[+z]||0)+(v||0) })); return d
}
function buildCrews(pickers, routes, mode='spread') {
  ...
  const groups=[]
  const flushGroup=(groupZones,cumAdj,assign)=>{ assign=Math.min(Math.max(assign,1),2,peopleLeft); groups.push({zones:[...groupZones],count:assign,cs:rawCs(groupZones)}); peopleLeft-=assign }
  ...
  if(peopleLeft>0){
    ...
    while(i<remainZones.length&&peopleLeft>0){
      if(peopleLeft===2){
        ...
        for(let j=0;j<rest.length-1;j++){ accAdj+=adjDemand[rest[j]];accLocs+=ZONE_LOCS[rest[j]]||0; const g2Locs=locs(rest.slice(j+1)),imbalance=Math.abs(2*accAdj-totalAdj); const locPenalty=(accLocs>MAX_LOCS_PER_GROUP||g2Locs>MAX_LOCS_PER_GROUP)?1e9:0; const score=imbalance+locPenalty;if(score<bestScore){bestScore=score;bestCut=j} }
      ...
      if(groupZones.length>0&&peopleLeft>1&&cumLocs+zLocs>MAX_LOCS_PER_GROUP){ const assign=(cumAdj>=csTarget*assignThreshold&&peopleLeft>=2)?2:1; flushGroup(groupZones,cumAdj,assign);groupZones=[];cumAdj=0;cumLocs=0; if(peopleLeft<=0){const extra=remainZones.slice(i);groups[groups.length-1].zones.push(...extra);groups[groups.length-1].cs+=rawCs(extra);break}; continue }
      ...
      const singleHeavy=span===1&&cumAdj>=csTarget*SOLO_THRESHOLD,pairNatural=span===2&&cumAdj>=csTarget*2*PAIR_CUT_MIN
      ...
      if(shouldCut){ const assign=(cumAdj>=csTarget*assignThreshold&&peopleLeft>=2)?2:1; flushGroup(groupZones,cumAdj,assign);groupZones=[];cumAdj=0;cumLocs=0; if(peopleLeft<=0){if(i<remainZones.length){const extra=remainZones.slice(i);groups[groups.length-1].zones.push(...extra);groups[groups.length-1].cs+=rawCs(extra)}break} }
    }
  }
  const crews=[ {label:'1 person — pallet/Z2',zones:[1,2],...CREW_PALETTE[0],flex:'covers Z1 + Z2',count:1,cs:palletCs}, {label:'2 people — Z3–4',zones:[3,4],...CREW_PALETTE[1],flex:'flex ↔ Z2 / Z5',count:z34People,cs:z34Cs} ]
  if(z5Assigned) crews.push({label:'1 person — Z5',zones:[5],...CREW_PALETTE[2],flex:'flex ↔ Z4 / Z6',count:1,cs:z5Cs})
  groups.forEach((g,i)=>{ const zFirst=g.zones[0],zLast=g.zones[g.zones.length-1]; const zLabel=g.zones.length===1?`Z${zFirst}`:`Z${zFirst}–${zLast}`; const prevZ=i===0?5:groups[i-1].zones[groups[i-1].zones.length-1]; const nextZ=i<groups.length-1?groups[i+1].zones[0]:null; crews.push({label:`${p(g.count)} — ${zLabel}`,zones:g.zones,...CREW_PALETTE[(i+3)%CREW_PALETTE.length],flex:nextZ?`flex ↔ Z${prevZ} / Z${nextZ}`:`flex → Z${prevZ}`,count:g.count,cs:g.cs}) })
  return crews
}

// ─── Picker tile (draggable) ──────────────────────────────────────────────────
function PickerTile({ picker, onRemove }) {
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({ id: picker.employee_id, data: { picker } })
  return (
    <div ref={setNodeRef} {...listeners} {...attributes}
      style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background: isDragging ? '#dbeafe' : '#fff', border: `1px solid ${isDragging ? '#1565C0' : '#ccc'}`, borderRadius: 3, padding: '2px 5px', fontSize: 10, cursor: 'grab', opacity: isDragging ? 0.4 : 1, marginBottom: 2, userSelect: 'none', touchAction: 'none', whiteSpace: 'nowrap' }}>
      <span style={{ overflow:'hidden', textOverflow:'ellipsis', flex:1, minWidth:0 }}>{picker.name}</span>
      <button onPointerDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();onRemove(picker.employee_id)}} style={{ marginLeft:3, background:'none', border:'none', color:'#bbb', cursor:'pointer', fontSize:10, padding:'0 1px', flexShrink:0 }} title="Remove">×</button>
    </div>
  )
}

// ─── Droppable crew cell ──────────────────────────────────────────────────────
function CrewDropCell({ crewIdx, crew, pickersForCrew, onRemove, children }) {
  const { isOver, setNodeRef } = useDroppable({ id: `crew-${crewIdx}` })
  return (
    <td ref={setNodeRef} colSpan={crew.zones.length}
      style={{ background: isOver ? '#dbeafe' : crew.border, border: `2px solid ${isOver ? '#1565C0' : '#555'}`, padding: '4px 6px', textAlign: 'center', whiteSpace: 'nowrap', verticalAlign: 'top', transition: 'background 0.1s' }}>
      {children}
      <div style={{ marginTop: pickersForCrew.length ? 4 : 0 }}>
        {pickersForCrew.map(p => <PickerTile key={p.employee_id} picker={p} onRemove={onRemove} />)}
        {pickersForCrew.length === 0 && <div style={{ fontSize:9, color:'#aaa', fontStyle:'italic', marginTop:2 }}>drop picker here</div>}
      </div>
    </td>
  )
}

// ─── Unassigned droppable pool ────────────────────────────────────────────────
function UnassignedPool({ pickers, onRemove }) {
  const { isOver, setNodeRef } = useDroppable({ id: 'crew-unassigned' })
  if (pickers.length === 0) return null
  return (
    <tr>
      <td colSpan={7} style={{ background:'#f5f5f5', border:'1px solid #ddd', padding:'4px 8px', fontSize:10, color:'#666', fontWeight:'bold', position:'sticky', left:0, zIndex:2 }}>Unassigned pickers</td>
      <td ref={setNodeRef} colSpan={12} style={{ background: isOver ? '#dbeafe' : '#f9f9f9', border:`1px solid ${isOver?'#1565C0':'#ddd'}`, padding:'4px 6px', transition:'background 0.1s' }}>
...
function PickScheduleEditor({ scheduleRows, onRowUpdate, onRowAdd, onRowDelete }) {
  const [activeDay, setActiveDay] = useState('Monday'), [saving, setSaving] = useState(null), [addingDay, setAddingDay] = useState(null), [newRow, setNewRow] = useState({})
  ...
  const thS={background:'#37474F',color:'#fff',padding:'4px 6px',fontSize:10,border:'1px solid #546E7A',textAlign:'center',whiteSpace:'nowrap'}
  const tdS={border:'1px solid #dde',padding:'3px 5px',fontSize:11,background:'#fff',textAlign:'center',verticalAlign:'middle'}
  return (
    <div style={{fontFamily:'Arial, sans-serif',fontSize:11}}>
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{borderCollapse:'collapse',fontSize:11,width:'100%'}}>
          <thead><tr><th style={thS}>Seq</th><th style={thS}>Route #</th><th style={{...thS,textAlign:'left',paddingLeft:8}}>Route Name</th><th style={{...thS,textAlign:'left',paddingLeft:8}}>Description</th><th style={thS}>Appt Start</th><th style={thS}>Appt End</th><th style={{...thS,textAlign:'left',paddingLeft:8}}>Carrier</th><th style={thS}>Drop / Live</th><th style={thS}></th></tr></thead>
          <tbody>
            {dayRows.map((row,i)=>(
              <tr key={row.id} style={{background:i%2===0?'#fff':'#f9fafb'}}>
                <td style={{...tdS,color:'#888',width:36}}><TextCell value={String(row.pick_seq)} width={32} onSave={v=>handleUpdate(row,'pick_seq',parseInt(v)||row.pick_seq)}/></td>
                <td style={{...tdS,fontWeight:'bold',width:60}}><TextCell value={String(row.route_number)} width={48} onSave={v=>handleUpdate(row,'route_number',parseInt(v)||row.route_number)}/></td>
                <td style={{...tdS,textAlign:'left',paddingLeft:8}}><TextCell value={row.route_name} width={110} onSave={v=>handleUpdate(row,'route_name',v)}/></td>
                <td style={{...tdS,textAlign:'left',paddingLeft:8}}><TextCell value={row.description} width={130} onSave={v=>handleUpdate(row,'description',v)}/></td>
                <td style={tdS}><TimeCell value={row.appt_start_mins} onSave={v=>handleUpdate(row,'appt_start_mins',v)}/></td>
                <td style={tdS}><TimeCell value={row.appt_end_mins} onSave={v=>handleUpdate(row,'appt_end_mins',v)}/></td>
                <td style={{...tdS,textAlign:'left',paddingLeft:8}}><TextCell value={row.carrier} width={90} onSave={v=>handleUpdate(row,'carrier',v)}/></td>
                <td style={tdS}><DropLiveCell value={row.drop_or_live} onSave={v=>handleUpdate(row,'drop_or_live',v)}/></td>
                <td style={{...tdS,width:28}}>{saving===row.id?<span style={{color:'#888',fontSize:9}}>💾</span>:<button onClick={()=>handleDelete(row.id)} style={{fontSize:10,padding:'1px 4px',background:'none',border:'1px solid #ffcdd2',borderRadius:3,color:'#C62828',cursor:'pointer'}}>✕</button>}</td>
              </tr>
            ))}
            {addingDay===activeDay?(
              <tr style={{background:'#E8F5E9'}}>
                <td style={tdS}><span style={{color:'#888',fontSize:10}}>new</span></td>
                <td style={tdS}><input value={newRow.route_number||''} onChange={e=>setNewRow(p=>({...p,route_number:e.target.value}))} placeholder="Rt #" style={{width:48,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><input value={newRow.route_name||''} onChange={e=>setNewRow(p=>({...p,route_name:e.target.value}))} placeholder="Name" style={{width:100,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><input value={newRow.description||''} onChange={e=>setNewRow(p=>({...p,description:e.target.value}))} placeholder="Description" style={{width:120,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><input value={newRow.appt_start||''} onChange={e=>setNewRow(p=>({...p,appt_start:e.target.value}))} placeholder="5:30am" style={{width:62,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><input value={newRow.appt_end||''} onChange={e=>setNewRow(p=>({...p,appt_end:e.target.value}))} placeholder="7:30am" style={{width:62,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><input value={newRow.carrier||''} onChange={e=>setNewRow(p=>({...p,carrier:e.target.value}))} placeholder="Carrier" style={{width:80,fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}/></td>
                <td style={tdS}><select value={newRow.drop_or_live||'Drop'} onChange={e=>setNewRow(p=>({...p,drop_or_live:e.target.value}))} style={{fontSize:11,padding:'1px 3px',border:'1px solid #a5d6a7',borderRadius:3}}><option>Drop</option><option>Live</option></select></td>
                <td style={tdS}><div style={{display:'flex',gap:3}}><button onClick={handleAdd} style={{fontSize:10,padding:'2px 6px',background:'#2E7D32',color:'#fff',border:'none',borderRadius:3,cursor:'pointer'}}>Save</button><button onClick={()=>{setAddingDay(null);setNewRow({})}} style={{fontSize:10,padding:'2px 6px',background:'#eee',border:'1px solid #ccc',borderRadius:3,cursor:'pointer'}}>✕</button></div></td>
              </tr>
            ):(
              <tr><td colSpan={9} style={{...tdS,background:'#f9f9f9'}}><button onClick={()=>setAddingDay(activeDay)} style={{fontSize:11,padding:'3px 12px',background:'#fff',border:'1px dashed #90CAF9',borderRadius:4,color:'#1565C0',cursor:'pointer'}}>+ Add route to {activeDay}</button></td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function UploadArea({ onSnapshot, scheduleRows, planDate }) {
  const [dragging,setDragging]=useState(false),[file,setFile]=useState(null),[parsing,setParsing]=useState(false),[parseError,setParseError]=useState(null),[omniPulling,setOmniPulling]=useState(false)
  const inputRef=useRef(null)
  function handleFiles(fileList){const f=Array.from(fileList).find(f=>/\.(xlsx|xls|csv)$/i.test(f.name));if(f){setFile(f);setParseError(null)}}
  async function handleLoad(){
    if(!file) return; setParsing(true); setParseError(null)
    try{const XLSX=await import('https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs');const buf=await file.arrayBuffer();const wb=XLSX.read(buf,{type:'array',cellDates:true});wb._XLSX=XLSX;const snap=parsePicklineXlsx(wb,scheduleRows);if(!snap.routes||snap.routes.length===0) throw new Error('No routes found — check that the Cases sheet has data');onSnapshot(snap)}catch(err){setParseError(err.message??'Failed to parse file')}finally{setParsing(false)}
  }
  async function handleOmniPull(){
    setOmniPulling(true);setParseError(null)
    try{
      const res=await fetch('/.netlify/functions/omni-pickline',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date:planDate})})
      if(!res.ok){const e=await res.json().catch(()=>({}));throw new Error(e.error||`Omni request failed (${res.status})`)}
      const{casesRows,tieHighRows,shortageRows}=await res.json()
      const snap=buildSnapshotFromOmni(casesRows,tieHighRows,shortageRows,planDate,scheduleRows)
      if(!snap.routes||snap.routes.length===0) throw new Error(`No routes found in Omni for ${planDate} — orders may not be in system yet`)
      onSnapshot(snap)
    }catch(err){setParseError(err.message??'Omni pull failed')}finally{setOmniPulling(false)}
  }
  ...
  return(
    <div style={{maxWidth:540,margin:'40px auto',fontFamily:'Arial, sans-serif'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}><div style={{flex:1,height:1,background:'#e0e0e0'}}/><span style={{fontSize:11,color:'#aaa',whiteSpace:'nowrap'}}>or upload Excel file</span><div style={{flex:1,height:1,background:'#e0e0e0'}}/></div>
      ...
      <button onClick={handleLoad} disabled={!file||parsing} style={{width:'100%',padding:'12px',fontSize:14,fontWeight:'bold',background:file?'#1565C0':'#ccc',color:'#fff',border:'none',borderRadius:6,cursor:file?'pointer':'not-allowed'}}>{parsing?'Loading…':'Load Pick Brief'}</button>
    </div>
  )
}

// ─── Pick Table ───────────────────────────────────────────────────────────────
function PickTable({ pickers, cpmh, tl, netCs, hourOverrides, setHourOverride, routes, crews, crewMode, setCrewMode, pickerRoster, activePickerId, onPickerDragEnd, onPickerDragStart, onPickerDragCancel, onPickerRemove, onPickerSync, syncingB2e, syncMsg }) {
  ...
  for(const p of pickerRoster){
    if(p.zone==null){pickersByCrew.unassigned.push(p)}
    else{
      const ci=zoneToCrewIdx[p.zone]
      if(ci!=null) pickersByCrew[ci].push(p)
      else pickersByCrew.unassigned.push(p)
    }
  }
  ...
  const totalCs=routes.reduce((s,r)=>s+(r.cs||0),0),totalBaseCases=paceRows.slice(0,PACE.length).reduce((s,r)=>s+r.thisCases,0),hasOverrides=Object.keys(hourOverrides).length>0
  ...
  const activePicker = activePickerId ? pickerRoster.find(p=>p.employee_id===activePickerId) : null
  ...
  return(
    <div style={{marginBottom:20}}>
      ...
      <div style={{overflowX:'auto'}}>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={({active})=>onPickerDragStart(active.id)} onDragEnd={onPickerDragEnd} onDragCancel={onPickerDragCancel}>
              ...
              <td colSpan={NUM_LEFT} style={{background:'#263238',border:'1px solid #555',padding:'4px 6px',position:'sticky',left:0,zIndex:3}}>
                <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                  ...
                  <div style={{display:'flex',gap:3,marginLeft:'auto',alignItems:'center'}}>
                    {syncMsg&&<span style={{fontSize:9,color:syncMsg.err?'#ef9a9a':'#A5D6A7',maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{syncMsg.text}</span>}
                    <button onClick={onPickerSync} disabled={syncingB2e} style={{fontSize:9,padding:'2px 7px',borderRadius:3,cursor:syncingB2e?'not-allowed':'pointer',border:'none',background:syncingB2e?'#546E7A':'#1565C0',color:'#fff'}}>{syncingB2e?'Syncing…':'↻ Sync 206s'}</button>
                    {[['spread','Spread'],['standard','Standard']].map(([m,lbl])=>(<button key={m} onClick={()=>setCrewMode(m)} style={{fontSize:9,padding:'1px 6px',borderRadius:3,cursor:'pointer',border:crewMode===m?'1px solid #90CAF9':'1px solid #546E7A',background:crewMode===m?'#1565C0':'#37474F',color:crewMode===m?'#fff':'#90A4AE',fontWeight:crewMode===m?'bold':'normal'}}>{lbl}</button>))}
                  </div>
                </div>
              </td>
              {crews.map((crew,ci)=>(
                <CrewDropCell key={crew.zones[0]} crewIdx={ci} crew={crew} pickersForCrew={pickersByCrew[ci]||[]} onRemove={onPickerRemove}>
        ...
        <DragOverlay>{activePicker&&<div style={{background:'#1565C0',color:'#fff',borderRadius:4,padding:'4px 10px',fontSize:11,boxShadow:'0 4px 12px rgba(0,0,0,0.25)',cursor:'grabbing'}}>{activePicker.name}</div>}</DragOverlay>
        </DndContext>
      </div>
    </div>
  )
...
export default function PicklinePanel({ snapshot, hourOverrides, onSnapshot, onOverridesChange, onClear, planDate }) {
  ...
  useEffect(() => {
    ...
    fetchPickerAssignments().then(rows => {
      setPickerRoster(rows.map(r => ({ employee_id: r.employee_id, name: r.employee_name, zone: r.zone ?? null })))
    })
  }, [])
  ...
  const handlePickerSync = useCallback(async () => {
    ...
    try {
      ...
      const savedZones = {}
      for (const p of pickerRoster) savedZones[p.employee_id] = p.zone ?? null
      const merged = b2ePickers.map(e => ({ employee_id: String(e.id), name: e.name, zone: savedZones[String(e.id)] ?? null }))
      await Promise.all(merged.map(p => upsertPickerAssignment(p.employee_id, p.name, p.zone)))
      setPickerRoster(merged)
      setSyncMsg({ err: false, text: `Synced ${merged.length} pickers` })
    } catch (err) {
  ...
  }, [pickerRoster, pickers, snapshot, crewMode])

  const handlePickerRemove = useCallback(async (employeeId) => {
    await deletePickerAssignment(employeeId)
    setPickerRoster(prev => prev.filter(p => p.employee_id !== employeeId))
  }, [])
          ...
          {!snapshot ? (
            ...
            <div>
              ...
              <PickTable
                ...
                pickerRoster={pickerRoster}
                activePickerId={activePickerId}
                onPickerDragStart={setActivePickerId}
                onPickerDragEnd={handlePickerDragEnd}
                onPickerDragCancel={()=>setActivePickerId(null)}
                onPickerRemove={handlePickerRemove}
                onPickerSync={handlePickerSync}
                syncingB2e={syncingB2e}
                syncMsg={syncMsg}
              />
