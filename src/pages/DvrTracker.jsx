...
function DetailModal({incident,fac,onClose,onUpdate,onDelete}) {
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:'var(--bg0)',border:'1px solid var(--border)',borderRadius:12,width:680,maxWidth:'95vw',maxHeight:'90vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,.3)'}}>
        ...
        <div style={{padding:20,overflowY:'auto',flex:1}}>
          ...
          <div style={{border:'1px solid var(--border)',borderRadius:8,padding:14,marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:600,color:'var(--text-secondary)',marginBottom:10,display:'flex',alignItems:'center',gap:5}}>
              ...
              Investigation — {fields.invOpen?<span style={{color:'#7c3aed'}}>Needed</span>:<span style={{color:'#16a34a'}}>Done</span>}
              <span style={{fontSize:10,color:'var(--text-secondary)',fontWeight:400,marginLeft:4}}>(blank = not done)</span>
            </div>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:'var(--bg0)',border:'1px solid var(--border)',borderRadius:12,width:660,maxWidth:'95vw',maxHeight:'90vh',overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,.3)'}}>
        ...
        <div style={{padding:20,overflowY:'auto',flex:1}}>
          ...
          {row('Investigation notes',<textarea style={{...inp,minHeight:50,resize:'vertical'}} value={f.investigationNotes} onChange={e=>set('investigationNotes',e.target.value)} placeholder="Video watched, root cause, findings..."/>)}
          {sdiv('Adjustment')}
        ...
        <div style={{padding:'12px 20px',borderTop:'1px solid var(--border)',display:'flex',justifyContent:'flex-end',gap:8,flexShrink:0,background:'var(--bg1)'}}>
          ...
          <button onClick={()=>{const adjOpen=!f.adjBy;const coachingOpen=f.coachingRequired==='Yes'&&!f.coachingDate;const invOpen=!f.investigationNotes;onAdd(f.fac,{...f,adjOpen,coachingOpen,invOpen});onClose()}} style={{background:'var(--text-primary)',border:'1px solid var(--text-primary)',borderRadius:6,padding:'6px 16px',cursor:'pointer',color:'var(--bg0)',fontSize:13,fontFamily:'inherit',fontWeight:600}}>Save incident</button>
        </div>
      </div>
    </div>
  )
...
export default function DvrTracker() {
  ...
  const filtered=useMemo(()=>{
    ...
    if(statusFilter==='inv')items=items.filter(i=>i.invOpen)
  ...
  const handleAdd=useCallback((fac,fields)=>{
    ...
    const prefix=prefixes[fac]||'DVR';const base=bases[fac]||1000
            ...
            <div style={{background:'var(--bg1)',border:'1px solid var(--border)',borderRadius:10,padding:'14px 16px',marginBottom:16}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                ...
                <select value={statusFilter} onChange={e=>{setStatusFilter(e.target.value);setPage(1)}} style={{padding:'5px 10px',fontSize:12,border:'1px solid var(--border)',borderRadius:6,background:'var(--bg0)',color:'var(--text-primary)',fontFamily:'inherit'}}>
                  ...
                  <option value="inv">Investigation pending</option>
                  <option value="both">Adj + Coaching open</option>
                </select>
                    ...
                    {loading&&!paged.length?(
                      ...
                      <tr key={i.id} onClick={()=>setDetail({id:i.id,fac:i._fac})} style={{background:idx%2===0?'var(--bg0)':'var(--bg1)',cursor:'pointer'}}
                        ...
                        <td style={{...tdBase,maxWidth:180}}>
                          ...
                          {i.investigationNotes&&<div style={{fontSize:11,color:'#7c3aed',marginTop:3,lineHeight:1.4}}>🔍 {preview(i.investigationNotes,40)}</div>}
                        </td>
        ...
        {tab==='dash'&&(
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:12,marginBottom:20}}>
              {[['Total open',activeData.length,'var(--red)'],['Adj. needed',adjOpen,'#d97706'],['Coaching pending',coachOpen,'var(--red)'],['Inv. pending',invPending,'#7c3aed']].map(([l,v,c])=>(
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:16,marginBottom:20}}>
                {[['cal','Caledonia','#1d4ed8',cal],['ken','Kenosha','#15803d',ken],['mad','Madison','#7c3aed',mad]].map(([id,name,color,data])=>(
                  <div key={id} style={{border:`1px solid var(--border)`,borderTop:`3px solid ${color}`,borderRadius:8,padding:16,background:'var(--bg0)'}}>
                    ...
                    {[['Total open',data.length,'var(--red)'],['Adj. needed',data.filter(i=>i.adjOpen).length,'#d97706'],['Coaching pending',data.filter(i=>i.coachingOpen).length,'var(--red)'],['Inv. pending',data.filter(i=>i.invOpen).length,'#7c3aed']].map(([lbl,val,c])=>(
