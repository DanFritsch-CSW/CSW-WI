...
// and again to back one row per FEFO project (facility='ken',
// dashboard_type='fefo_<projectId>') — see fefo-digest-run.cjs.
//
...
// "Skip-to-next-valid-day lookahead" note for the full mechanism. NOTE:
// this column is inert for FEFO rows (fefo-digest-run.cjs doesn't read
// it) since FEFO's content date === fire date, so there's no lookahead
// concept to opt into — see fefo-digest-run.cjs header.
...
// targets tomorrow's date for the appointment-based digests — see the
// function's own header for why this is safe to leave open, same reasoning
// as triggerDailyDiscussionTest above). functionName is
// 'prepick-digest-run', 'wr-cases-digest-run', 'dailyops-digest-run', or
// 'fefo-digest-run'.
//
// body (added 2026-07-14, FEFO per-project digests) — optional extra
// payload merged into the POST body. Needed for shared digest functions
// that back MULTIPLE settings rows (fefo-digest-run.cjs backs one row per
// FEFO project, all sharing the same function), so the manual test needs
// to say which row/project to run for (e.g. { dashboardType: 'fefo_faioa5' }).
// Defaults to {} so every existing caller (single-row digests) is
// unaffected — same '{}' body as before.
export async function triggerDigestTest(functionName, body = {}) {
